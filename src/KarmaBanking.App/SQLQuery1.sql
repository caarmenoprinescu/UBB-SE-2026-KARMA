...
*/
IF
NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'KarmaBankingDb')
BEGIN
    CREATE
DATABASE KarmaBankingDb;
END
GO
...
IF
OBJECT_ID('SavingsTransaction', 'U') IS NOT NULL
DROP TABLE SavingsTransaction;
IF
OBJECT_ID('AutoDeposit', 'U') IS NOT NULL
DROP TABLE AutoDeposit;
IF
OBJECT_ID('ChatAttachment', 'U') IS NOT NULL
DROP TABLE ChatAttachment;
IF
OBJECT_ID('ChatMessage', 'U') IS NOT NULL
DROP TABLE ChatMessage;
IF
OBJECT_ID('InvestmentTransaction', 'U') IS NOT NULL
DROP TABLE InvestmentTransaction;
IF
OBJECT_ID('InvestmentHolding', 'U') IS NOT NULL
DROP TABLE InvestmentHolding;
IF
OBJECT_ID('Portfolio', 'U') IS NOT NULL
DROP TABLE Portfolio;
IF
OBJECT_ID('AmortizationRow', 'U') IS NOT NULL
DROP TABLE AmortizationRow;
IF
OBJECT_ID('Loan', 'U') IS NOT NULL
DROP TABLE Loan;
IF
OBJECT_ID('SavingsAccount', 'U') IS NOT NULL
DROP TABLE SavingsAccount;
IF
OBJECT_ID('ChatSession', 'U') IS NOT NULL
DROP TABLE ChatSession;
IF
OBJECT_ID('LoanApplication', 'U') IS NOT NULL
...
INSERT INTO Portfolio (userId, totalValue, totalGainLoss, gainLossPercent)
VALUES (1, 25000, 2000, 8);

-- Holding-uri pentru Portofoliul de mai sus
INSERT INTO InvestmentHolding (portfolioId, ticker, assetType, quantity, avgPurchasePrice, currentPrice,
                               unrealizedGainLoss)
VALUES (1, 'AAPL', 'Stock', 10, 150, 180, 300),
...
INSERT INTO SavingsAccount (userId, savingsType, balance, accruedInterest, apy, accountStatus, accountName,
                            targetAmount, targetDate)
VALUES (1, 'Standard', 5000, 120, 0.025, 'Active', 'Emergency Fund', 10000, '2026-12-01');

-- Mesaj de succes
PRINT
'Baza de date KarmaBankingDb a fost creată și populată cu succes!';
...
WHERE portfolioId = (SELECT id FROM Portfolio WHERE userId = 1);
